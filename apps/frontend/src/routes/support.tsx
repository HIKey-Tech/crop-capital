        ...
        <div className="premium-container pt-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-border mb-24">
            {[
              ...
              return (
                <div key={i} className="bg-background p-10 lg:p-12 group hover:bg-primary/[0.025] transition-colors duration-300">
                  ...
                  <h3 className="text-xl font-black text-foreground mb-3 uppercase tracking-tight">{card.title}</h3>
                  <p className="text-base text-muted-foreground mb-8 leading-relaxed font-medium">{card.desc}</p>
                  <a href={card.href}
          ...
          <div className="max-w-3xl">
            <div className="flex items-center gap-4 mb-10">
              ...
              <span className="text-[11px] font-black uppercase tracking-[0.5em] text-primary">Common Questions</span>
            </div>
            <h2 className="text-display text-[clamp(2rem,4vw,4.5rem)] text-foreground mb-12">
              Frequently Asked <span className="italic-serif text-primary">Questions.</span>
            </h2>
              ...
              {faqs.map((faq, i) => (
                <div key={i} className="border-b border-border last:border-b-0">
                  <button
                    ...
                    className="w-full flex items-start justify-between gap-8 p-8 text-left group hover:bg-primary/[0.025] transition-colors duration-200 cursor-pointer"
                  >
                    <span className="text-base font-black text-foreground uppercase tracking-tight group-hover:text-primary transition-colors">
                      {faq.q}
                    </span>
                    <ChevronRight className={`w-5 h-5 text-primary flex-shrink-0 mt-0.5 transition-transform duration-300 ${openFaq === i ? 'rotate-90' : ''}`} />
                  </button>
                  {openFaq === i && (
                    <div className="px-8 pb-8">
                      <p className="text-base text-muted-foreground leading-relaxed font-medium">{faq.a}</p>
                    </div>
                  )}
                </div>
              ))}
          ...
          <div className="mt-24 bg-muted/40 border border-border p-12 lg:p-16 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-10">
            <div>
              <div className="flex items-center gap-4 mb-4">
                ...
                <span className="text-[11px] font-black uppercase tracking-[0.5em] text-primary">Still need help?</span>
              </div>
